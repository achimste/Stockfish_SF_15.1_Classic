...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
      ...
      if constexpr (!SearchMate)
      {
          ...
          if (!rootNode)
          {
              ...
              if (Threads.stop.load(std::memory_order_relaxed)
                  || pos.is_draw(ss->ply)
                  || ss->ply >= MAX_PLY)
                  return (ss->ply >= MAX_PLY && !ss->inCheck) ? evaluate<SearchMate>(pos)
                  ...
                  && ttValue < probCutBeta))
          {
              ...
              while ((move = mp.next_move<SearchMate>()) != MOVE_NONE)
      ...
      moves_loop: // When in check, search starts here
          ...
          while ((move = mp.next_move<SearchMate>(moveCountPruning)) != MOVE_NONE)
          {
              assert(is_ok(move));
                  ...
                  && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
              {
                  ...
                  else
                  {
                      ...
                      if (!pos.see_ge(move, Value((-24 * lmrDepth - 15) * lmrDepth)))
                          continue;
                  }
              }
          ...
          while ((move = mp.next_move<SearchMate>(moveCountPruning)) != MOVE_NONE)
          {
                  ...
                  && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
              {
                  ...
                  else
                  {
                      ...
                      if (!pos.see_ge(move, Value((-27 * lmrDepth - 16) * lmrDepth)))
                          continue;
                  }
              }
      ...
      if constexpr (!SearchMate)
      {
          ...
          else
          {
              if (ss->ttHit)
              {
                  ...
                  (ss - 1)->currentMove != MOVE_NULL ? evaluate<SearchMate>(pos) : -(ss - 1)->staticEval;
