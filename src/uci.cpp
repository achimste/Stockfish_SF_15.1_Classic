  ...
  void test_mate() {
      ...
      if (is)
      {
          ...
          while (!is.eof())
          {
              ...
              // Write the mating pv line
              Thread* bestThread = Threads.get_best_thread();
              TimePoint elapsed_time = now() - time;
              std::string san = SAN::to_san(bestThread->rootMoves[0], pos);
              csv << elapsed_time << ";" << san << std::endl;
              std::cout << std::endl;
          }
      }
  }
